...
#define CRITICAL_SECTION_START  unsigned int oldInterrupts = noInterrupts();
#define CRITICAL_SECTION_END    restoreInterrupts(oldInterrupts);
...
//    uint16_t *table_address = (uint16_t *)&speed_lookuptable_fast[(uint8_t)(step_rate>>8)][0];
    ...
    uint16_t gain = speed_lookuptable_fast[step_rate>>8][0];
    MultiU16X8toH16(timer, tmp_step_rate, gain);
    timer = speed_lookuptable_fast[step_rate>>8][1] - timer;
  }
  else
  { // lower step rates
    uint16_t *table_address = (uint16_t *)&speed_lookuptable_slow[0][0];
    table_address += ((step_rate)>>1) & 0xfffc;
    timer = (uint16_t)*table_address;
    timer -= (((uint16_t)(*table_address+2) * (uint8_t)(step_rate & 0x0007))>>3);
  }
